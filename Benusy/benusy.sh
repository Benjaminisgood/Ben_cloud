...
ensure_deps() {
  if ! (cd "$API_DIR" && PYTHONPATH=src python -c "import benusy_api, fastapi, sqlalchemy, gunicorn" >/dev/null 2>&1); then
...
ensure_db_migrated() {
  ...
  (cd "$API_DIR" && PYTHONPATH=src "$VENV_DIR/bin/alembic" upgrade head)
}
...
start_scheduler() {
  ...
  (
    cd "$API_DIR"
    PYTHONPATH=src nohup python -m "$SCHEDULER_MODULE" \
...
check_port_free() {
  if [ -n "$(list_listening_pids)" ]; then
    ...
    if [ -n "$(collect_project_port_pids)" ]; then
      warn "Port appears to be occupied by this project; run './benusy.sh stop' first."
    fi
