...
from ..repositories.photos_repo import list_photos
...
def sync_missing_photos_from_oss(db: Session) -> int:
    ...
    try:
        ...
        logger.exception("Failed to list OSS objects for Benphoto auto-import.")
        return 0
    ...
    existing_paths = set()
    for photo in list_photos(db, limit=10_000):
        existing_paths.add(photo.oss_path)
        existing_paths.add(normalize_oss_path(settings, photo.oss_path))
