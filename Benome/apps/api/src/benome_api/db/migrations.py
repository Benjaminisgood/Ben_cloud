...
from benome_api.db.session import engine
...
def ensure_migration_state() -> None:
    ...
    with engine.connect() as conn:
        ...
        if not has_version_table:
            ...
            if app_tables:
                ...
                return
            raise RuntimeError("Database schema is not initialized. Run `make db-upgrade` before starting the app.")

        rows = conn.execute(text("SELECT version_num FROM alembic_version")).fetchall()
        current_revisions = {str(row[0]).strip() for row in rows if row and row[0]}
        if not current_revisions:
            raise RuntimeError("alembic_version table is empty. Run `make db-upgrade` to initialize schema version.")
