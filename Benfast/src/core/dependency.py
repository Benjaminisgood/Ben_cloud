...
def get_current_username(
    ...
    # Benbot SSO session cookie can access docs/openapi without Basic Auth.
    cookie_token = request.cookies.get(settings.SSO_TOKEN_COOKIE_NAME, "")
    if cookie_token:
        try:
            payload = jwt.decode(
                cookie_token,
            ...
            if payload.get("token_type") == "access" and payload.get("user_id"):
                return f"sso-user-{payload['user_id']}"
...
class AuthControl:
    ...
    async def is_authed(
        ...
        try:
            raw_token = ""
            # 优先使用 Authorization Bearer；若缺失则回退 SSO Cookie。
            if token is not None and token.credentials:
                raw_token = token.credentials
            else:
                raw_token = request.cookies.get(settings.SSO_TOKEN_COOKIE_NAME, "")
