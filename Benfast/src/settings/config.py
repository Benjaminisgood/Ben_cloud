...
class Settings(BaseSettings):
    ...
    DOCS_SITE_DIR: str = os.getenv("DOCS_SITE_DIR", os.path.join(BASE_DIR, "site"))
    SQLITE_DB_PATH: str = os.getenv(
        "SQLITE_DB_PATH", os.path.join(DATA_DIR, "benfast.sqlite3")
    )
    ...
    SSO_REFRESH_COOKIE_NAME: str = os.getenv(
        "SSO_REFRESH_COOKIE_NAME", "benfast_refresh_token"
    )
    ADMIN_USERNAME: str = os.getenv("ADMIN_USERNAME", "benbenbuben")
    ADMIN_PASSWORD: str = os.getenv("ADMIN_PASSWORD", "benbenbuben")
    LABDOCS_STORAGE_BACKEND: str = os.getenv("LABDOCS_STORAGE_BACKEND", "local")
    LABDOCS_STORAGE_PREFIX: str = os.getenv("LABDOCS_STORAGE_PREFIX", "benfast")
    LABDOCS_LOCAL_ROOT: str = os.getenv(
        "LABDOCS_LOCAL_ROOT", os.path.join(DATA_DIR, "labbooks")
    )
    LABDOCS_DOCS_SOURCE_ROOT: str = os.getenv(
        "LABDOCS_DOCS_SOURCE_ROOT", os.path.join(DATA_DIR, "docs_site_source")
    )
    LABDOCS_PUBLISH_ROOT: str = os.getenv(
        "LABDOCS_PUBLISH_ROOT", os.path.join(DOCS_SITE_DIR, "books")
    )
    ALIYUN_OSS_ENDPOINT: str = os.getenv("ALIYUN_OSS_ENDPOINT", "")
    ALIYUN_OSS_ACCESS_KEY_ID: str = os.getenv("ALIYUN_OSS_ACCESS_KEY_ID", "")
    ALIYUN_OSS_ACCESS_KEY_SECRET: str = os.getenv(
    ...
    @field_validator("SSO_REDIRECT_PATH")
    @classmethod
    ...
    @field_validator("LABDOCS_STORAGE_BACKEND")
    @classmethod
    def validate_labdocs_storage_backend(cls, v):
        value = str(v or "local").strip().lower()
        if value not in {"local", "aliyun_oss"}:
            raise ValueError("LABDOCS_STORAGE_BACKEND 仅支持 local 或 aliyun_oss")
        return value
