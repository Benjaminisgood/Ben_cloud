...
def client(tmp_path: Path):
    db_path = tmp_path / "Benprefs-test.sqlite"
    source_db = Path(__file__).resolve().parents[4] / "database" / "preferences.db"
    ...
    import benprefs_api.core.config as config_module

    config_module.get_settings.cache_clear()
    _run_migrations(os.environ["DATABASE_URL"])

    import benprefs_api.db.session as session_module

    session_module = importlib.reload(session_module)

    import benprefs_api.main as main_module
