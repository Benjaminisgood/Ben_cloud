...
def client(tmp_path: Path):
    db_path = tmp_path / "Benhealth-test.sqlite"
    source_db = Path(__file__).resolve().parents[4] / "database" / "health.db"
    ...
    import benhealth_api.core.config as config_module

    config_module.get_settings.cache_clear()
    _run_migrations(os.environ["DATABASE_URL"])

    import benhealth_api.db.session as session_module

    session_module = importlib.reload(session_module)

    import benhealth_api.main as main_module
